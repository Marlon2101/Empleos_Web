...
const renderValoraciones = (items) => {
  if (!items || items.length === 0) {
    listaValoraciones.innerHTML = `<p class="text-muted mb-0">Tu empresa todavía no tiene reseñas.</p>`;
    return;
  }
...
const cargarResenas = async () => {
  try {
    ...
    const response = await fetch(`${API_URL}/valoraciones/empresa/${empresa.id_empresa}`, {
      headers: {
    ...
    if (!response.ok) {
      showAlert(data.mensaje || "No se pudieron cargar las reseñas");
      return;
    }
